...
def _simple_pdf(title: str, subtitle: str, body_lines: list[str]) -> bytes:
    ...
    pdf = (
        ...
        f"BT /F1 14 Tf 72 760 Td ({title}) T* /F1 10 Tf ({subtitle}) "
        f"T* T* ({content}) Tj ET\n"
        "endstream endobj\n"
...
async def risks_pdf(
    ...
    for r in rows:
        body.append(
            "{title} [status: {status}] threat: {threat} vuln: {vuln} "
            "impact {impact} likelihood {like} treatment: {treat}".format(
                title=r["title"],
                status=r["status"],
                threat=r["threat"] or "-",
                vuln=r["vulnerability"] or "-",
                impact=r["impact"] or "-",
                like=r["likelihood"] or "-",
