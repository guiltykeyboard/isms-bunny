...
async def registration_options(
    ...
    # store challenge in settings store keyed by user (simplified for demo)
    await set_setting(session, f"webauthn_reg_{user.id}", options.challenge.decode())
    return options
...
async def registration_verify(
    ...
    try:
        verify_webauthn_registration(credential, stored.encode())
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"Verification failed: {exc}")
    await set_setting(session, f"webauthn_cred_{user.id}", credential)
...
async def login_options(
    ...
    await set_setting(session, f"webauthn_auth_{user_id}", options.challenge.decode())
    return options
